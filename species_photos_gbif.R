...
# for the WECA region
sp_media_list <- map(taxon_keys, ~get_media(.x, geometry = geometry,
                                            limit = 30)) |> 
...
make_file_tbl <- function(media_tbl){
  media_tbl |> 
  filter(format == "image/jpeg",
         !is.na(identifier)) |> 
  inner_join(species_tbl |> select(common_name, usage_key),
             by = "usage_key") |>
mutate(license_short = str_extract(license, "by.*/{1}$"),
       across(.cols = c(license_short, common_name, rightsHolder),
              ~map_chr(.x, make_clean_names)),
       filepath = glue("data/species_images/{common_name}__{usage_key}/{common_name}__{license_short}__{rightsHolder}__.jpg"))
}
...
folder_names <- species_tbl |> 
  transmute(folder = glue("data/species_images/{make_clean_names(common_name)}__{usage_key}")) |> 
  pull()
...
dl_files <- function(media_file_tbl){
  walk2(media_file_tbl$identifier,
