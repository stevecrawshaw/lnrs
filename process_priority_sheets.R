...
parse_priorities_tbl <- function(sheets_list, areas_count) {
  ...
  nms <- c("theme", "priority_id", "biodiversity_priority", 1:areas_count)
...
parse_measures_tbl <- function(sheets_list) {
  ...
  measures_tbl %>%
    mutate(measure = coalesce(recommended_measures, recommended_measure)) %>%
    rownames_to_column(var = "measure_id") %>%
    mutate(
      across(
        .cols = c(level_of_ambition, land_type),
        .fn = ~ if_else(.x == "N/A", NA_character_, .x)
      ),
      # remove text descriptions and add a PH1 placeholder
      # designation pending assignation of correct codes
      countryside_stewardship = if_else(
        !str_starts(countryside_stewardship, "\\b[A-Z]{2}\\d{1,2}\\b"),
        "PH1",
      ...
      countryside_stewardship = if_else(
        countryside_stewardship == "NA",
        NA_character_,
        countryside_stewardship
      )
    ) %>%
    separate_longer_delim(
      cols = c(stakeholder, countryside_stewardship, sfi),
      delim = "; "
    ) %>%
    pivot_longer(
      starts_with("associated_priority"),
      values_to = "priority_id"
    ) %>%
    filter(!is.na(priority_id)) %>%
    select(-name, -starts_with("recommended")) %>%
    # line below fails if done above!

    separate_longer_delim(associated_area_codes, delim = "; ") %>%
    pivot_longer(
      cols = c(countryside_stewardship, sfi, woodland, other),
    ...
    select(
      measure_id,
      measure,
      ambition = level_of_ambition,
      land_type,
      stakeholder_type = stakeholder,
      area_id = associated_area_codes,
      priority_id,
      guidance = link_to_further_guidance,
      everything()
    ) %>%
    mutate(across(.cols = ends_with("_id"), as.integer))
}
...
make_species_area_priority_lookup_tbl <- function(priority_species_tbl) {
  priority_species_tbl %>%
    separate_longer_delim(cols = most_relevant_areas, delim = "; ") %>%
    separate_longer_delim(cols = relevant_priorities, delim = "; ") %>%
    rename(area_id = most_relevant_areas, priority_id = relevant_priorities) %>%
    distinct(species_id, area_id, priority_id) %>%
    mutate(
      across(
        everything(),
        ~ if_else(!str_starts(.x, "[0-9]"), NA_integer_, as.integer(.x))
      )
    )
}
...
parse_cs_grant_codes <- function(sheets_list) {
  sheets_list %>%
    ...
    mutate(
      category = str_to_sentence(meaning),
      meaning = NULL,
      scheme = NULL
    ) %>%
...
make_cs_tbl <- function(links_raw_tbl, domain = "https://www.gov.uk") {
  ...
  links_raw_tbl %>%
    ...
    filter(str_detect(
      grant_name,
      pattern = "^[A-Z]{2}\\d{0,2}:|^[A-Z]{2}\\d{0,2}\\s"
    )) %>%
    mutate(
      url = glue("{domain}{url}"),
      text = NULL,
...
clean_sfi_tbl <- function(
  sfi_raw_tbl,
  url = "https://assets.publishing.service.gov.uk/media/6516c0986a423b0014f4c62e/SFI23_handbook.pdf"
) {
  sfi_raw_tbl %>%
    mutate(
      url = url,
    ...
    select(url, grant_name, grant_id, grant_scheme, annual_payment, category)
}
...
make_all_grants_tbl <- function(cs_tbl, sfi_tbl, cs_grant_codes_tbl) {
  bind_rows(cs_tbl, sfi_tbl) %>%
    mutate(
      code_prefix = if_else(
        str_starts(grant_id, "[A-Z]{2}"),
        str_extract_all(grant_id, "[A-Z]") %>%
          map_chr(~ paste0(.x, collapse = "")),
        ""
      )
    ) %>%
    left_join(cs_grant_codes_tbl, by = join_by(code_prefix == code_prefix)) %>%
    mutate(
      grant_focus = coalesce(category.x, category.y),
...
priority_area_lookup_tbl <- parse_priorities_tbl(
  sheets_list,
  areas_count = 53
) %>%
  distinct(area_id, priority_id) %>%
  filter(
    area_id %in% areas_tbl$area_id,
...
all_grants_tbl <- make_all_grants_tbl(cs_tbl, sfi_tbl, cs_grant_codes_tbl)
