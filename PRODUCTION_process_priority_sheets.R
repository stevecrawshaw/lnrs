...
scheme_check_url_tbl <- interim_areas_tbl |> 
  ...
  mutate(status = map_chr(local_funding_schemes, linkchecker)) |> 
  nest_by(status)
...
# a vector of area id's with underscores and trailling numbers removed
