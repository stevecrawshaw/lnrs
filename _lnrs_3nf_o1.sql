...
-- SELECT * FROM memory.glimpse(source_table);
...
SELECT * FROM memory.glimpse(area);
...
SELECT * FROM memory.glimpse(grant_table);
...
FROM memory.glimpse(measure_has_type);
...
FROM memory.glimpse(source_table_recreated_vw);
...
FROM memory.glimpse(apmg_slim_vw);
