pacman::p_load(tidyverse, glue, janitor, magick, collapse, fs)
...
images_raw_tbl <- sub_folders |>
  map(
    ~ fs::dir_ls(.x, glob = "*.jpg|*.png") |>
      path_split()
  ) |>
  collapse::unlist2d()

image_upload_path = fs::path("data", "portal_upload", "images_for_upload")

images_out_tbl <- images_raw_tbl |>
  select(1, 6, 7) |>
  set_names("full_path", "folder", "file_name") |>
  mutate(
    folder_components_list = map(folder, ~ str_split(., "__")),
  ...
  mutate(
    amended_file_name = map_chr(
      file_name,
      ~ str_replace_all(.x, c("_B" = "__B", "(?<!_)_by " = "__by__"))
    ),
    final_file_name = map_chr(
      amended_file_name,
      ~ str_replace_all(.x, c("^(.*?(BY|BC).*?)(_)" = "\\1__", "'" = "_"))
    )
  ) |>
  mutate(
    license = map_chr(final_file_name, ~ str_split_i(.x, pattern = "__", 2)),
    attribution = map_chr(
      final_file_name,
      ~ str_split_i(.x, pattern = "__", 3) |> str_remove("\\.jpg|\\.png")
    ),
    full_file_path = glue("{full_path}/{file_name}"),
    write_path_full = glue(
      "{image_upload_path}/{usage_key}__{final_file_name}"
    ),
    write_path_thumbnail = glue(
      "{image_upload_path}/{usage_key}__thumbnail__{final_file_name}"
    )
  )
...
images_out_tbl |>
  write_csv(fs::path("data", "portal_upload", "images_out_tbl.csv"))
